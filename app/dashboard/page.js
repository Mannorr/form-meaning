...
export default function DashboardPage({ user = {}, announcements = [], events: serverEvents = [], content = [], newMembers = [] }) {
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: c.navBg, backdropFilter: "blur(20px) saturate(180%)", borderBottom: `1px solid ${c.border}` }}>
        <div style={{ ...maxW, display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          ...
          <nav className="fm-desk-nav" style={{ display: "flex", alignItems: "center", gap: 2 }}>
            {navLinks.map(link => (
              <button key={link.key} onClick={() => window.location.href=link.href} style={{ ...btnGhost, color: link.key === "dashboard" ? c.red : c.textMuted, fontWeight: link.key === "dashboard" ? 700 : 500, gap: 6 }}>
                <span style={{ color: link.key === "dashboard" ? c.red : c.textSoft }}>{link.icon}</span>{link.label}
              </button>
            ))}
        ...
        {menuOpen && (
          <div style={{ position: "fixed", inset: 0, top: 60, zIndex: 99, background: c.bg, padding: 24, animation: "fadeIn 0.15s ease", display: "flex", flexDirection: "column", gap: 4 }}>
            {navLinks.map(link => (
              <button key={link.key} onClick={() => { setMenuOpen(false); window.location.href=link.href; }} style={{ ...btnGhost, fontSize: 17, padding: "16px 8px", width: "100%", justifyContent: "flex-start", gap: 12, color: link.key === "dashboard" ? c.red : c.textMuted, fontWeight: link.key === "dashboard" ? 700 : 500 }}>{link.icon} {link.label}</button>
            ))}

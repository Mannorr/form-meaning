...
export default async function Dashboard() {
  ...
  const { data: events } = await supabaseAdmin.from("events").select("*").eq("status", "upcoming").order("date", { ascending: true }).limit(4);
  ...
  // Fetch new members (include email for fallback display)
  const { data: newMembers } = await supabaseAdmin.from("memberships").select("name, email, discipline, created_at").eq("status", "active").order("created_at", { ascending: false }).limit(4);
